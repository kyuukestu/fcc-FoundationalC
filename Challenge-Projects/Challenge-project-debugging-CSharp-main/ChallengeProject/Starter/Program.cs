...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    if (changeNeeded < 0) // Cost greater than amount paid.
    {
        cashTill[3] -= twenties;
        cashTill[2] -= tens;
        cashTill[1] -= fives;
    ...
    while ((changeNeeded > 0) && (cashTill[0] >= changeNeeded))
    {
        cashTill[0]--;
        changeNeeded -= 1;
    ...
    if (changeNeeded > 0)
    {
        ...
        cashTill[3] += twentiesGiven;
        cashTill[2] += tensGiven;
        cashTill[1] += fivesGiven;
        cashTill[0] += onesGiven;
